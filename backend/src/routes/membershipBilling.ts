...
membershipBillingRouter.post("/membership/checkout",requireAuth,async(req:AuthedRequest,res,next)=>{
  try {
    const user=findUserById(req.userId!);
    if(user?.role!=="Engineer")return res.status(403).json({error:"Paid memberships are available to engineer accounts only."});
    const parsed=schema.safeParse(req.body);
    if(!parsed.success)return res.status(400).json({error:"Choose a paid engineer membership plan."});
    assertStripeMembershipConfigured(parsed.data.plan);
    if(hasOpenMembershipInvoice(listMembershipInvoices(req.userId!),parsed.data.plan))return res.status(409).json({error:"An open invoice already exists for this membership plan."});
    const invoice=createMembershipInvoice(req.userId!,parsed.data.plan,plans[parsed.data.plan],"GBP");
    const invoiceId=membershipInvoiceId(invoice);
    const checkout=await createStripeMembershipCheckout({userId:req.userId!,email:user.email,plan:parsed.data.plan,invoiceId});
    createMembershipCheckoutSession(checkout.id,invoiceId,req.userId!,parsed.data.plan);
    return res.status(201).json({invoice,checkoutUrl:checkout.url});
  } catch(error:unknown) {
    next(error);
  }
});
