...
export function readInclusivePreferences(profile: EngineerProfile | Record<string, unknown>): InclusivePreferences {
  ...
  if (!raw || typeof raw !== 'object') return structuredClone(DEFAULT_INCLUSIVE_PREFERENCES);
...
export function matchesWorkPreference(
  ...
  const workModeMatches = !filter.workMode || filter.workMode === 'any' || preferences.workModes.includes(filter.workMode);
