...
const accountRequestError = (reason: unknown) => errorMessage(reason, "The account request could not be completed.");
...
export function ForgotPasswordPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(accountRequestError(reason));
...
export function ResetPasswordPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    } catch (reason: unknown) { setMessage({ error: accountRequestError(reason), success: "" }); }
  }
...
export function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    apiService.confirmEmailVerification(token).then(() => setMessage("Email verified successfully.")).catch((reason: unknown) => setError(accountRequestError(reason)));
  }, [token]);
...
export function AccountSecurityPage() {
  ...
  async function downloadDataExport() {
    ...
    try {
      ...
      setMessage({ error: accountRequestError(reason), success: "" });
    } finally {
      setExporting(false);
    }
  }
