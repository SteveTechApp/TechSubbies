...
export const ProfileCompliance = ({ formData, setFormData }: ProfileComplianceProps) => {
    
    const data = formData.compliance;

    const handleComplianceChange = (field: keyof Compliance, value: any) => {
        setFormData((prev: EngineerProfile) => ({
            ...prev,
            compliance: {
                ...prev.compliance,
                [field]: value
    ...
    return (
        <div className="space-y-6">
            <p className="text-sm text-gray-500">Provide details about your insurance, competencies, and equipment. This builds trust with companies.</p>
            ...
            <div className="space-y-6">
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                        ...
                        {data.professionalIndemnity.hasCoverage && (
                            <div className="space-y-3 pl-8 animate-fade-in-up">
                                ...
                                <FileUploadInput label="Upload Certificate" fileUrl={data.professionalIndemnity.certificateUrl} onFileChange={url => handleInsuranceChange('professionalIndemnity', 'certificateUrl', url)} isVerified={data.professionalIndemnity.isVerified}/>
                            </div>
                        )}
                    </div>
                     <div className="space-y-3 p-4 border rounded-lg">
                        ...
                        {data.publicLiability.hasCoverage && (
                            <div className="space-y-3 pl-8 animate-fade-in-up">
                                ...
                                <FileUploadInput label="Upload Certificate" fileUrl={data.publicLiability.certificateUrl} onFileChange={url => handleInsuranceChange('publicLiability', 'certificateUrl', url)} isVerified={data.publicLiability.isVerified}/>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {/* Competency Section */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <CompetencySlider label="Power Tool Competency" value={data.powerToolCompetency || 0} onChange={val => handleComplianceChange('powerToolCompetency', val)} />
                    <CompetencySlider label="Access Equipment Training" value={data.accessEquipmentTrained || 0} onChange={val => handleComplianceChange('accessEquipmentTrained', val)} />
                </div>
